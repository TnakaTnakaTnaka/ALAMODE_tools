...
import extract_openmx_mod as openmx
...
usage = "usage: %prog [options] openmxrun*.nout (or *.xml or *.pw.out or *.str)"
...
parser.add_option('--offset',
                  help="Specify an output file (either *.nout *.xml, *.pw.out, or *.str) of an\
...
if __name__ == "__main__":
    ...
    if len(file_results) == 0:
        print "Usage: extract.py [options] openmxrun *.nout \
         (*.xml or *.pw.out, or *.str)"
        print
...
$ python displace.py -h"
    ...
    if options.OpenMX is None and options.VASP is None and options.QE is None and options.xTAPP is None:
        print "Error : Either --OpenMX, --VASP, --QE, or --xTAPP option must be given."
        exit(1)

    elif options.OpenMX and options.VASP or options.OpenMX and options.QE or \
      options.OpenMX and options.xTAPP or options.VASP and options.QE or \
      options.VASP and options.xTAPP or options.QE and options.xTAPP:
        print "Error : --OpenMX, --VASP, --QE, and --xTAPP \
